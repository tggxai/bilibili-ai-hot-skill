...
一个可复用的 Codex Skill：每日抓取 B站的「综合热门」「全站排行榜」，按周抓取最新一期「每周必看」，把主题相关视频归为 AI 软件、科技硬件 / 3C 或 AIGC 内容，并在本次实际检查的热门来源里识别可与主题重叠的视频播客机会，最后把结果写成按日期累积的飞书日报。它也可以持续采样综合热门，分析榜单的实际更新规律。
...
“视频播客机会”既不是第四个主题分类，也不是第四个固定数据源。Skill 只从本次实际检查的热门来源中识别播客形态：标题或独立标签明确写有“视频播客/播客/Podcast”，或者视频至少 20 分钟且同时具备对谈与期数、主播/嘉宾结构。时长、“上B站看播客”等活动标签和 UP 主历史都不能单独作为结论。

## 功能

- 完整翻页抓取综合热门并读取全站排行榜；每周必看默认只在周五 18:00 后抓取，其他运行明确显示“本次未检查”。
- 结合标题、简介、分区和 B站标签判断视频是否与 AI 有关。
- 排除反诈提示、禁用 AI 声明、“不是 AI”和作者名中的偶然字符串。
- 同一 BV 号去重，同时保留它出现过的榜单和榜位。
- 生成「日期一级标题 + 三个主分类 + 视频播客机会」的飞书日报。
...
三个主题分类表格包含来源与榜位、产品 / 客户标签、视频链接、UP 主和判定依据。视频播客表格额外显示主题、播放量与机会依据。日期下方列出三个热门来源的规模与分类数量，以及播客候选池、机会数和多榜交叉数。

## 视频播客样本怎么使用

用户提供的 B站官方专区前 30 页表格共有 360 支节目、155 位博主。它是一次性的离线校准样本，用来观察值得收录的视频播客通常有哪些格式信号，不是每天要抓取的内容池，也不是日报白名单。

- 标题或独立标签明确写有“视频播客/播客/Podcast”时，可直接判为播客形态。
- 没有显式播客标记时，必须同时具备长时长、对话/访谈信号，以及期数或主播/嘉宾结构。
- 时长、同一 UP 主反复出现、活动标签和历史专区收录都不能单独证明当前视频值得入报。
- 日报是否收录的第一道门槛是视频当前进入综合热门、全站排行榜或每周必看；样本只帮助判断它是不是播客形态。
...
默认 `--weekly-mode auto` 只在周五 18:00 后抓取每周必看。周五任务因限流失败时，可在确认该期尚未成功写入后，于周六 10:00 显式补查：

```bash
python3 scripts/collect_and_write.py --weekly-mode fetch
```

调试时也可以强制跳过：

```bash
python3 scripts/collect_and_write.py --weekly-mode skip
```

如果当前网络被 B站标签接口限流，可在另一个受信任网络完成 `collect()`，再把完整报告 JSON 通过标准输入交给本机写入。导入报告必须属于当天，且三个主题、播客候选池、机会数、多榜交叉数和明细必须一致；飞书凭证无需离开本机。
...
- `popular`、`ranking`、`weekly`：热门来源的规模与主分类数量；`weekly.checked` 区分“本次未检查”和真实零命中
- `podcast`：本次来源去重候选数、播客机会数、多榜交叉数和主题重叠数
- `unique_software_count`：本次来源去重后的 AI 软件数量
- `unique_hardware_count`：本次来源去重后的科技硬件 / 3C 数量
- `unique_aigc_count`：本次来源去重后的 AIGC 数量
- `unique_related_count`：本次来源中全部入报视频的去重数量
...
```text
使用 $track-bilibili-ai-hot 执行监测，每天抓取综合热门、全站排行榜，仅在周五 18:00 后抓取最新每周必看，
区分“AI 软件”“科技硬件 / 3C”与“AIGC 内容”，并在本次实际来源中另列可重叠的视频播客机会，覆盖消费数码、创客工程和航空航天，标注产品、品牌和明确披露的客户，
并写入或刷新配置的飞书文档当天章节。
...
- 综合热门会随时间和访问会话变化；日报记录的是本次实际来源快照。官方视频播客专区样本不参与日常抓取。
