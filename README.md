...
一个可复用的 Codex Skill：每日抓取 B站的「综合热门」「全站排行榜」、最新一期「每周必看」和官方「视频播客专区」，把主题相关视频归为 AI 软件、科技硬件 / 3C 或 AIGC 内容，另列可与三个主题重叠的视频播客机会，并把结果写成按日期累积的飞书日报。它也可以持续采样综合热门，分析榜单的实际更新规律。
...
“视频播客机会”不是第四个主题分类。Skill 会完整抓取 B站官方视频播客播单作为格式白名单，日报展示专区头部 12 支，并补入同时进入综合热门、排行榜或每周必看的播客。专区位置是编辑排序，不会被表述为播放量排名。专区之外只有在标题或独立标签明确写有“播客/Podcast”，或长对谈同时具备期数、主播/嘉宾结构时才纳入；“上B站看播客”等活动标签不能单独作为结论。

## 功能

- 完整翻页抓取综合热门、官方视频播客播单，并读取全站排行榜和最新每周必看。
...
- 标签接口失败超过 5%，或播客池实际条数与接口声明不一致时拒绝写入，避免把限流或半页数据造成的漏数当成真实数据。
...
三个主题分类表格包含来源与榜位、产品 / 客户标签、视频链接、UP 主和判定依据。视频播客表格额外显示主题、播放量与机会依据。日期下方还有四个来源的规模与分类数量摘要。

## 视频播客口径为什么这样定

用于校准的 B站官方专区前 30 页样本共有 360 支节目、155 位博主：63.1% 的标题直接出现“播客/Podcast”，90.8% 的节目时长不少于 30 分钟，但仍有 24.2% 的标题没有“播客、对话、期数”任一强标记。因此：

- 官方专区收录是最可靠的格式证据。
- 时长只能辅助判断，不能单独证明是播客。
- 同一 UP 主反复出现不是必要条件，否则会漏掉被专区单次推荐的新节目。
- 专区第一页代表当前编辑推荐与新鲜度；进入三大热门来源代表额外的全站热度确认。
...
如果当前网络被 B站标签接口限流，可在另一个受信任网络完成 `collect()`，再把完整报告 JSON 通过标准输入交给本机写入。导入报告必须属于当天，且三个主题、播客机会和专区完整性字段必须与明细一致；飞书凭证无需离开本机。
...
- `popular`、`ranking`、`weekly`：三个热门来源的规模与主分类数量
- `podcast`：官方播客池规模、机会数、头部推荐数、三榜交叉数和主题重叠数
- `unique_software_count`：四源去重后的 AI 软件数量
- `unique_hardware_count`：四源去重后的科技硬件 / 3C 数量
- `unique_aigc_count`：四源去重后的 AIGC 数量
- `unique_related_count`：四源去重后的全部入报视频数量
...
```text
使用 $track-bilibili-ai-hot 执行每日监测，抓取综合热门、全站排行榜、最新每周必看和官方视频播客专区，
区分“AI 软件”“科技硬件 / 3C”与“AIGC 内容”，另列可重叠的视频播客机会，覆盖消费数码、创客工程和航空航天，标注产品、品牌和明确披露的客户，
并写入或刷新配置的飞书文档当天章节。
...
- 综合热门会随时间和访问会话变化；视频播客专区是编辑播单而非播放量排行榜。日报记录的是运行时快照。
