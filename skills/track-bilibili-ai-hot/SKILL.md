...
description: Collect the current Bilibili 综合热门、全站排行榜、每周必看 and official 视频播客专区, classify videos into mutually exclusive AI software, technology hardware/3C, and AIGC content categories, maintain a separate overlapping video-podcast opportunity track, identify product/brand and explicitly disclosed client labels, deduplicate them, append a date-first Feishu report, and sample the popular list to study its update cadence. Use for B站 AI/科技数码热门日报、热门播客或视频播客机会监测、AI 软件监测、AI 与科技硬件、3C 数码、创客工程、机械航模、航空航天、AIGC 榜单统计、产品或甲方客户识别、热门更新规律观测、每日视频监测、复跑某天报告，或把榜单结果写入飞书文档。
---

# B站 AI 热门日报

运行确定性脚本抓取榜单与 B站官方视频播客专区、读取视频标签并过滤误命中。每支主题相关视频只归入一个主分类：“AI 软件”“科技硬件 / 3C”“AIGC 内容”；“视频播客机会”是可与三个主分类重叠的内容形态，并单独标注可识别的产品、品牌和明确披露的甲方客户。B站分区只作为候选来源，不直接作为报告分类。写入时通过 `--doc` 或 `BILIBILI_AI_FEISHU_DOC` 指定目标飞书文档。
...
3. 检查脚本返回的 JSON：
   ...
   - 标签接口失败超过 5%，或官方播客池实际条数与接口声明不一致时，脚本主动失败，避免把限速或半页数据造成的漏数写进日报。
...
- **视频播客机会**不是第四个主题分类。先完整抓取 B站官方“视频播客专区”播单作为格式白名单，再把专区头部 12 支与当前综合热门、排行榜、每周必看的播客交集纳入机会轨道。专区位置是编辑排序，不写成播放量排名。
- 官方专区收录优先于标题启发式：30 页样本中有一部分播客标题不含“播客”、对话或期数标记，因此不能用时长、标题关键词或同一 UP 主出现次数替代专区白名单。专区之外只接受标题或独立标签明确写有“播客/Podcast”，或至少 20 分钟且具备对谈与期数、主播/嘉宾结构；“上B站看播客”等活动标签不能单独作为结论。
...
- 同一 BV 号在同一来源只计一次；四个来源按 BV 号合并，播客轨道与三个主题的交集不重复增加主题去重数。
...
输入报告必须是当天数据，且三个主题与播客机会的去重数量、专区完整性字段和明细一致，否则脚本拒绝写入。飞书用户凭证仍只保留在执行 `--upsert` 的本机。
