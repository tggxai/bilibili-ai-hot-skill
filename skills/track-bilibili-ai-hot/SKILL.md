...
运行确定性脚本抓取综合热门和全站排行榜，并按独立周节奏抓取最新每周必看，读取视频标签并过滤误命中。每支主题相关视频只归入一个主分类：“AI 软件”“科技硬件 / 3C”“AIGC 内容”；“视频播客机会”只在本次实际检查的热门来源中识别，是可与三个主分类重叠的内容形态。B站分区只作为候选来源，不直接作为报告分类。写入时通过 `--doc` 或 `BILIBILI_AI_FEISHU_DOC` 指定目标飞书文档。
...
- 排行榜使用全站榜单。每周必看不随每天两次日报重复抓取：`auto` 模式只在周五 18:00 后抓取最新一期；若该次因限流失败，周六 10:00 的恢复任务先确认周五未成功，再显式使用 `--weekly-mode fetch` 补查。其他运行跳过该来源，不复用旧一期冒充当天更新。
- **视频播客机会**不是第四个数据源或主题分类。它只扫描本次实际检查的综合热门、全站排行榜和每周必看视频，并把符合播客形态的条目标为可重叠机会；不得固定加入官方专区视频，也不得把专区位置当成热度。
